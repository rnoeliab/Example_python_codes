...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun  1 09:03:54 2020
...
cross_start = CoordPair(lat=-24.5, lon=-48.5)
cross_end = CoordPair(lat=-23., lon=-44.8)
...
dbz_levels = np.arange(to_np(dbz).min(),to_np(dbz).max()-1, 3.)
...
dbz_contours = ax_cross.contourf(xs,ys,to_np(dbz_cross_filled), extend="max")
# Add the color bar
...
pyplot.savefig(output+"Cross_section_mountains.tif")
pyplot.show()